...
var guaiamumLocationLatitude = -8.1153065; 
var guaiamumLocationLongitude = -34.8936867;
var sushiLocationLatitude = -8.115305;
var sushiLocationLongitude = -34.893794;
var camaroesLocationLatitude = -8.116844;
var camaroesLocationLongitude = -34.893702;
var navegantesLocationLatitude = -8.115054;
var navegantesLocationLongitude = -34.893031;
var sertanejaLocationLatitude = -8.131740;
var sertanejaLocationLongitude = -34.900942;
var kostaLocationLatitude = -8.047812;
var kostaLocationLongitude = -34.892395;
var ruibarbosaLatitude = -8.0478363;
var ruibarbosaLongitude = -34.899657;
...
function changeIsland(){
	...
	var url = window.location;
        var ilha = url.pathname.split("/")[1];
        var page = url.pathname.split("/")[2];
        _current = ilha;
        _current_page = page;
				
        var _previous = window.sessionStorage.getItem("_previous");
        var _previous_page = window.sessionStorage.getItem("_previous_page");
        // alert(_previous + '...' + _current);
        // alert(_current.indexOf("ilha"));    
        if((_current != _previous) && ((_current.indexOf("ilha") > -1) || (_current.indexOf("ruibarbosa") > -1))){
            // window.location = window.location + '#';
            window.location.reload();
        }else if(_previous_page && _current_page !== undefined){
        	if((_current_page.indexOf("contato") > -1 || _current_page.indexOf("reservas") > -1) && (_previous_page != _current_page)){
	        // if((_previous_page.indexOf("contato") > -1 || _previous_page.indexOf("reservas") > -1) && (_current_page.indexOf("contato") > -1 || _current_page.indexOf("reservas") > -1) && (_previous_page != _current_page)){
	            window.location.reload();
	        }
        }
        window.sessionStorage.setItem("_previous", ilha);
				window.sessionStorage.setItem("_previous_page", page);
		// console.log(url.href, _current, _previous, _current_page)
		// if(url.href.search('cardapio') > 1 && _current_page != _previous_page){
		// 	window.location.reload()
		// }
}
...
function showIfood(){
	var url = window.location;
	var lista = url.pathname.split("/");

	if(lista[2] == ""){
		// alert(lista[2]);
		$("#ifood").attr("id", "ifood-home");
		$("#ifood-2").attr("id", "ifood-home-2");
	}else{
		$("#ifood-home").attr("id", "ifood");
		$("#ifood-home-2").attr("id", "ifood-2");
	}
}
		...
		$.ajax({
			...
			beforeSend: function(xhr, settings) {
				...
				validate = function() {
					...
					if (qtd_people.val().length === 0) {
						qtd_people.parent().addClass('error');
						errors = true;
					} else {
						qtd_people.parent().removeClass('error');
					}
				...
				$("#submit-reservation").text("Enviando ...");
				$("#submit-reservation").attr('disabled', true);

			},
			success: function(data){
				$("#submit-reservation").text("Enviar");
...
function initialize() {
	...
	// alert(site.indexOf('guaiamu'));
	if(site.indexOf('guaiamum') > 0){
		var markerPosition = new google.maps.LatLng(guaiamumLocationLatitude, guaiamumLocationLongitude);
	}else if(site.indexOf('camaroes') > 0){
		var markerPosition = new google.maps.LatLng(camaroesLocationLatitude, camaroesLocationLongitude);
	}else if(site.indexOf('dakosta') > 0){
		var markerPosition = new google.maps.LatLng(kostaLocationLatitude, kostaLocationLongitude);
	}else if(site.indexOf('ruibarbosa') > 0){
		var markerPosition = new google.maps.LatLng(ruibarbosaLatitude, ruibarbosaLongitude);
	}else if(site.indexOf('navegantes') > 0){
		var markerPosition = new google.maps.LatLng(navegantesLocationLatitude, navegantesLocationLongitude);
	}else if(site.indexOf('sertaneja') > 0){
		var markerPosition = new google.maps.LatLng(sertanejaLocationLatitude, sertanejaLocationLongitude);
	}else if(site.indexOf('sushi') > 0){
		var markerPosition = new google.maps.LatLng(sushiLocationLatitude, sushiLocationLongitude);
	}
	...
	var marker = new google.maps.Marker({
		...
		title: 'Localização',
		map: map,
		icon: { url: '/static/img/icon1.png', origin: new google.maps.Point(0, 0) },
		draggable: false
	});
}
...
function loadScript() {
	...
	script.src = 'https://maps.googleapis.com/maps/api/js?v=3&callback=initMap&key=AIzaSyAaJBDwl0ejhL3y_69WI9Ctk01Xx2pUAe8';
